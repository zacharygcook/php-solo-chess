...
echo "[1/10] Validating agent documentation"
"$ROOT/scripts/check-agent-docs.sh"

echo "[2/10] Rejecting oversized repository files"
"$ROOT/scripts/check-large-files.sh"

echo "[3/10] Checking technical-debt tracking"
"$ROOT/scripts/check-tech-debt.sh"

echo "[4/10] Linting PHP and JavaScript source"
"$ROOT/scripts/lint.sh"

echo "[5/10] Checking deterministic formatting"
"$ROOT/scripts/format.sh" --check

echo "[6/10] Type-checking PHP"
composer --working-dir="$ROOT" typecheck

echo "[7/10] Running rules-engine unit tests"
"$ROOT/scripts/test.sh"

echo "[8/10] Starting isolated server"
...
echo "[9/10] Loading both frontend URL forms and browser assets"
...
echo "[10/10] Playing e2 to e4 through the API"
