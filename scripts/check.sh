...
echo "[1/12] Validating agent documentation"
"$ROOT/scripts/check-agent-docs.sh"

echo "[2/12] Rejecting oversized repository files"
"$ROOT/scripts/check-large-files.sh"

echo "[3/12] Checking technical-debt tracking"
"$ROOT/scripts/check-tech-debt.sh"

echo "[4/12] Scanning for committed secrets"
"$ROOT/scripts/check-secrets.sh"

echo "[5/12] Checking dependency policy"
php "$ROOT/scripts/check-dependencies.php"

echo "[6/12] Linting PHP and JavaScript source"
"$ROOT/scripts/lint.sh"

echo "[7/12] Checking deterministic formatting"
"$ROOT/scripts/format.sh" --check

echo "[8/12] Type-checking PHP"
composer --working-dir="$ROOT" typecheck

echo "[9/12] Running rules-engine unit tests"
"$ROOT/scripts/test.sh"

echo "[10/12] Starting isolated server"
...
echo "[11/12] Loading both frontend URL forms and browser assets"
...
echo "[12/12] Playing e2 to e4 through the API"
