...
echo "[1/19] Validating agent documentation"
"$ROOT/scripts/check-agent-docs.sh"

echo "[2/19] Rejecting oversized repository files"
"$ROOT/scripts/check-large-files.sh"

echo "[3/19] Checking technical-debt tracking"
"$ROOT/scripts/check-tech-debt.sh"

echo "[4/19] Scanning for committed secrets"
"$ROOT/scripts/check-secrets.sh"

echo "[5/19] Checking dependency policy"
php "$ROOT/scripts/check-dependencies.php"

echo "[6/19] Generating local security review"
"$ROOT/scripts/security-review.sh" "$TEMP_DIR/security-review.md"

echo "[7/19] Validating generated API documentation"
php "$ROOT/scripts/generate-api-docs.php" --check

echo "[8/19] Generating release notes from Git history"
php "$ROOT/scripts/generate-release-notes.php" --to=HEAD --output="$TEMP_DIR/release-notes.md"

echo "[9/19] Linting PHP and JavaScript source"
"$ROOT/scripts/lint.sh"

echo "[10/19] Enforcing source naming conventions"
php "$ROOT/scripts/check-naming.php"

echo "[11/19] Enforcing architecture layer boundaries"
php "$ROOT/scripts/check-architecture.php"

echo "[12/19] Enforcing cyclomatic-complexity budget"
"$ROOT/scripts/check-complexity.sh"

echo "[13/19] Enforcing duplicate-code budget"
php "$ROOT/scripts/check-duplication.php" --check

echo "[14/19] Checking deterministic formatting"
"$ROOT/scripts/format.sh" --check

echo "[15/19] Type-checking PHP"
composer --working-dir="$ROOT" typecheck

echo "[16/19] Running rules-engine unit tests"
"$ROOT/scripts/test.sh"

echo "[17/19] Starting isolated server"
...
echo "[18/19] Loading both frontend URL forms and browser assets"
...
echo "[19/19] Playing e2 to e4 through the API"
