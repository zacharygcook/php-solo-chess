...
echo "[1/13] Validating agent documentation"
"$ROOT/scripts/check-agent-docs.sh"

echo "[2/13] Rejecting oversized repository files"
"$ROOT/scripts/check-large-files.sh"

echo "[3/13] Checking technical-debt tracking"
"$ROOT/scripts/check-tech-debt.sh"

echo "[4/13] Scanning for committed secrets"
"$ROOT/scripts/check-secrets.sh"

echo "[5/13] Checking dependency policy"
php "$ROOT/scripts/check-dependencies.php"

echo "[6/13] Generating local security review"
"$ROOT/scripts/security-review.sh" "$TEMP_DIR/security-review.md"

echo "[7/13] Linting PHP and JavaScript source"
"$ROOT/scripts/lint.sh"

echo "[8/13] Checking deterministic formatting"
"$ROOT/scripts/format.sh" --check

echo "[9/13] Type-checking PHP"
composer --working-dir="$ROOT" typecheck

echo "[10/13] Running rules-engine unit tests"
"$ROOT/scripts/test.sh"

echo "[11/13] Starting isolated server"
...
echo "[12/13] Loading both frontend URL forms and browser assets"
...
echo "[13/13] Playing e2 to e4 through the API"
