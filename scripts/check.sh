...
echo "[1/16] Validating agent documentation"
"$ROOT/scripts/check-agent-docs.sh"

echo "[2/16] Rejecting oversized repository files"
"$ROOT/scripts/check-large-files.sh"

echo "[3/16] Checking technical-debt tracking"
"$ROOT/scripts/check-tech-debt.sh"

echo "[4/16] Scanning for committed secrets"
"$ROOT/scripts/check-secrets.sh"

echo "[5/16] Checking dependency policy"
php "$ROOT/scripts/check-dependencies.php"

echo "[6/16] Generating local security review"
"$ROOT/scripts/security-review.sh" "$TEMP_DIR/security-review.md"

echo "[7/16] Validating generated API documentation"
php "$ROOT/scripts/generate-api-docs.php" --check

echo "[8/16] Generating release notes from Git history"
php "$ROOT/scripts/generate-release-notes.php" --to=HEAD --output="$TEMP_DIR/release-notes.md"

echo "[9/16] Linting PHP and JavaScript source"
"$ROOT/scripts/lint.sh"

echo "[10/16] Enforcing source naming conventions"
php "$ROOT/scripts/check-naming.php"

echo "[11/16] Checking deterministic formatting"
"$ROOT/scripts/format.sh" --check

echo "[12/16] Type-checking PHP"
composer --working-dir="$ROOT" typecheck

echo "[13/16] Running rules-engine unit tests"
"$ROOT/scripts/test.sh"

echo "[14/16] Starting isolated server"
...
echo "[15/16] Loading both frontend URL forms and browser assets"
...
echo "[16/16] Playing e2 to e4 through the API"
