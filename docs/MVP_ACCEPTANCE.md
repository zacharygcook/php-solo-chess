...
| 2 | gap | A person can complete representative untimed and timed games entirely through the browser without state divergence between UI, API, persistence, clocks, history, or PGN. | Partial browser path in `scripts/browser-smoke.sh`; API/unit coverage in `tests/GameApiTest.php`, `tests/GamePersistenceTest.php`, `tests/GameHistoryTest.php`, `tests/PgnControllerTest.php`, `tests/PgnIntegrationTest.php`, and the complete untimed saved-game evidence in `tests/UntimedJourneyTest.php`. | Chunk 3 must prove the complete timed-game journey. Chunk 4 must finish browser interaction hardening. |
...
| 5 | covered | Saved games export valid PGN that reproduces their moves, result, and final position. | `tests/PgnExporterTest.php`, `tests/PgnControllerTest.php`, `tests/PgnIntegrationTest.php`, `tests/UntimedJourneyTest.php`, `backend/src/Services/PgnVerifier.php`, and `./scripts/test.sh`. | Chunk 3 must include PGN agreement in the representative timed journey test. |
