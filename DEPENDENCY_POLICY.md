...
committed lock or machine-readable policy record. The browser runtime has no external JavaScript
dependency.
...
`config/dependency-policy.json`; `./scripts/check.sh` verifies every direct Composer tool against
that record.
